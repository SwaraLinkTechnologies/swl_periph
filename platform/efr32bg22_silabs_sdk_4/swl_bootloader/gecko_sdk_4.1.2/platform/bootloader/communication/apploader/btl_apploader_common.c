...
void bootloader_apploader_communication_init(void)
{
  ...
  if (USE_RANDOM_ADDRESS) {
      ...
      psa_status_t rc = mbedtls_psa_external_get_random(&ctx,&btAddress.address[0],6,&length);
      if(!rc)
      {

      }// exceptional error handling is beyond this example, rc==0 and length==6 should be verified
      btAddress.address[5] |= 0xc0; // As required by Bluetooth spec random addresses
